...
server <- function(input, output,session) {
  ...
  output$despesa1=renderPlot({
   ggplot(react1())+geom_line(aes(x=ano,y=dpj,colour="despesas por habitantes"))+
      geom_line(aes(ano,dpjio,colour="despesas por habitantes sem inativos"))+ 
      scale_colour_manual(values=c("despesas por habitantes"="blue",
                                   "despesas por habitantes sem inativos"="red"))+
      theme(legend.position="bottom")+labs(colour=" ")+
      ggtitle("Série histórica das despesas por habitantes")
  ...
  output$despesa2=renderPlot({
     ...
     ggplot(react2()) + 
      ...
      geom_bar(mapping = aes(x =as.numeric(ano),y=receitas/1000000,fill="Receita"),stat="identity")+
      scale_x_continuous( breaks=c(209,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019))+
      labs(x="Ano",y="Milhões",fill=" ")+
      scale_fill_manual(values=c("Receita"="blue",
                                 "Despesas"="red"))+
      
      theme(legend.position="bottom")+
      ggtitle("Série histórica das despesas e receitas")
  })
  react3=reactive({
  ...
  react4=reactive({
    d2=despesas%>%filter(justica %in% input$jus,sigla %in% input$tribunal,uf_sede %in% input$uf)%>%
      select(ano, i)%>%
      mutate(i=as.numeric(gsub(",",".",i)))
    d2
  })
  output$a1=renderPlot({
    ggplot(react4(), aes(x=factor(ano)))+geom_col(aes(y=i))+ggtitle("Arrecadação")
  })
 react5=reactive({
  ...
  output$a2=renderPlot({
    ggplot(react5())+geom_line(aes(x=ano,y=dk,colour="Despesas com capital"))+
      ...
      theme(legend.position="bottom")+labs(colour="",y=" ")+
      scale_x_continuous( breaks=c(209,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019))
  })
 react6=reactive({
   d5=despesas%>%
 ...
 output$ft1=renderPlot({
   ggplot(data=subset(react6(),count >= 0.001),
     ...
     coord_polar("y", start=0)+
     labs(fill="Justiça",y="",x="")+
     ggtitle("Despesa recurso Humanos")+
     scale_fill_brewer(palette="Set1")+
